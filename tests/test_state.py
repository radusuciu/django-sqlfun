...
def _create_op(name, body, result_type='integer', identity='a integer', previous=None):
    previous_kwargs = ''
    ...
    return (
        f"""sqlfun.operations.CreateFunction(
                name={name!r},
...
def test_replay_last_write_wins_across_migrations():
    ...
    op_v2_src, v2_sql = _create_op(
        'state_replaced_fn',
        'SELECT a + 1;',
        previous={'sql': v1_sql, 'identity': 'a integer', 'result_type': 'integer'},
    )
    path_v1 = write_test_migration(
        'test_project',
        '0902_state_replaced_v1',
        _migration([('test_project', '0001_initial')], op_v1_src),
    )
    path_v2 = write_test_migration(
        'test_project',
        '0903_state_replaced_v2',
...
def test_replay_drop_removes_entry():
    ...
    path_create = write_test_migration(
        'test_project',
        '0904_state_dropped_create',
        _migration([('test_project', '0001_initial')], op_src),
    )
    path_drop = write_test_migration(
        'test_project',
        '0905_state_dropped_drop',
...
def test_replay_uses_squashed_replacement():
    ...
    path_v1 = write_test_migration(
        'test_project',
        '0906_state_squash_v1',
        _migration([('test_project', '0001_initial')], op_v1_src),
    )
    path_v2 = write_test_migration(
        'test_project',
        '0907_state_squash_v2',
...
def test_replay_normalizes_hand_written_drop_names():
    ...
    path_create = write_test_migration(
        'test_project',
        '0910_state_cased_drop_create',
        _migration([('test_project', '0001_initial')], op_src),
    )
    path_drop = write_test_migration(
        'test_project',
        '0911_state_cased_drop',
